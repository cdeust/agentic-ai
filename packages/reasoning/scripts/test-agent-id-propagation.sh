...
TARGET="$(make_target_repo team)"
SHIM="$TMP/shim-p1/bin"; ENV_REC="$TMP/shim-p1/env.txt"
install_shim "$SHIM" "$ENV_REC"
( cd "$TARGET"; PATH="$SHIM:$PATH" "$SPAWN" engineer "dummy task" >/dev/null )
...
TARGET="$(make_target_repo genius)"
SHIM="$TMP/shim-p2/bin"; ENV_REC="$TMP/shim-p2/env.txt"
install_shim "$SHIM" "$ENV_REC"
( cd "$TARGET"; PATH="$SHIM:$PATH" "$SPAWN" feynman "dummy task" >/dev/null )
RECORDED="$(cat "$ENV_REC" 2>/dev/null || echo '')"
...
TARGET="$(make_target_repo fanout)"
SHIM="$TMP/shim-p3/bin"; ENV_REC="$TMP/shim-p3/env.txt"
install_shim "$SHIM" "$ENV_REC"
( cd "$TARGET"; export MEMORY_AGENT_ID="orchestrator"; PATH="$SHIM:$PATH" "$SPAWN" engineer "dummy task" >/dev/null )
RECORDED="$(cat "$ENV_REC" 2>/dev/null || echo '')"
