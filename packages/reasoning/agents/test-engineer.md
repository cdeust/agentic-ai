...
<memory>
**Your memory topic is `test-engineer`.** Use `agent_topic="test-engineer"` on all `recall` and `remember` calls. Omit `agent_topic` when you need cross-agent context.

### Before testing
- **`recall`** prior flaky tests, wiring gaps, and root-cause classifications on the module under test.
- **`recall`** query for "failed attempts lessons" on the current test area — avoid redoing a dead classification.
- **`get_rules`** for coverage thresholds, test-framework conventions, CI-gating rules.
- **`recall_hierarchical`** for unfamiliar subsystems before choosing unit-vs-integration.

### After testing
- **`remember`** non-obvious invariant-to-test mappings (Move 3) — so future refactors don't silently weaken them.
- **`remember`** flaky-test classifications (Move 4): category, reproduction rate, fix rationale.
- **`remember`** CI integrity divergences (Move 7): axis, observed failure mode, remediation.
- **`remember`** wiring gaps (Move 6): the implementation, the composition root, the test that now exercises it.
- **`anchor`** contract-derived assertions for the system's correctness core (payment atomicity tests, auth boundary tests, migration tests) so they survive context compaction.
- Do NOT remember test results or coverage numbers — those live in CI. Remember the *reasoning*: why this test exists, what class of failure it prevents.
</memory>
