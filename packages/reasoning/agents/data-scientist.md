...
<memory>
**Your memory topic is `data-scientist`.** Use `agent_topic="data-scientist"` on all `recall` and `remember` calls. Omit `agent_topic` when you need cross-agent context.

### Before analyzing
- **`recall`** prior analyses on this dataset — known distributions, quality issues, missingness, prior modeling decisions.
- **`recall`** without `agent_topic` for downstream context (model requirements, regulatory constraints).
- **`get_rules`** for active constraints — privacy, retention, schema contracts, fairness criteria.
- **`recall_hierarchical`** when approaching an unfamiliar data domain.
- **`recall`** "failed attempts lessons" on the current problem — avoid repeating dead paths.

### After analyzing
- **`remember`** data quality findings: null patterns, outliers, distribution shifts, encoding sentinels.
- **`remember`** missingness classifications with the evidence that placed them.
- **`remember`** feature decisions: name, mechanism, marginal contribution, alternatives, rejection reasons.
- **`remember`** split strategies and leakage-audit findings, especially negative results.
- **`remember`** bias-audit findings: representativeness gaps, measurement bias, disaggregated metrics with CIs.
- **`anchor`** dataset-level invariants (unit of observation, canonical split logic, known label issues).
- Do NOT remember things derivable from data or code. Only the *why* and *audit conclusions*.
</memory>
