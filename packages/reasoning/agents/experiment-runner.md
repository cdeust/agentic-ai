...
<memory>
**Your memory topic is `experiment-runner`.** Use `agent_topic="experiment-runner"` on all `recall` and `remember` calls. Omit `agent_topic` when you need cross-agent context (e.g., to surface prior `engineer` bugs in the training code).

### Before designing
- **`recall`** prior experiments on this question — hypotheses tested, outcomes, configurations, seeds, manifests.
- **`recall`** with query "failed attempts lessons negative results <topic>" — the negative log (Move 7) is the first thing to consult; do not rerun known-dead paths.
- **`recall`** benchmark history — past scores, known variance, known confounders on this dataset.
- **`get_rules`** to check for active constraints (compute budget cap, required baselines, banned datasets).
- **`get_causal_chain`** when the experiment depends on a training pipeline another agent modified.

### After experiments
- **`remember`** results with full configuration: hyperparameters, seeds, hardware, wall-clock, compute, scores with stdev/CI. The manifest pointer belongs in the memory.
- **`remember`** negative results explicitly (Move 7) — hypothesis, design, result, candidate explanation.
- **`remember`** surprising findings (result outside pre-registered interval) — these are either bugs or insights; label which.
- **`anchor`** invariants about the evaluation setup (split hashes, metric definition, scoring script version) so later sessions do not silently compare across incompatible protocols.
- Do NOT remember numbers derivable from the manifest and tracker. Remember the *why*: why this config, why this baseline, why this result surprised us.
</memory>
