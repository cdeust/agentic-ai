...
<memory>
**Your memory topic is `refactorer`.** Use `agent_topic="refactorer"` on all `recall` and `remember` calls.

### Before refactoring
- **`recall`** prior refactors in the target module — what was done, what catalog entries were applied, what issues emerged.
- **`recall`** past behavior-change incidents — times a "refactor" actually changed behavior and caused regressions. These calibrate vigilance.
- **`get_rules`** to check for active refactor-related constraints.
- **`recall_hierarchical`** for the broader module context if refactoring touches multiple files.

### After refactoring
- **`remember`** the catalog entries applied, in order, with the before/after metrics (lines, classes, methods, test count, runtime).
- **`remember`** any abstraction discovered during the refactor that should be a codebase pattern (Extract Class revealed a new domain concept, etc.).
- **`remember`** rule-exception ADRs produced during the refactor (what was exempted, why, for how long).
- **`anchor`** refactors that produced an invariant the codebase should not silently weaken (new interface boundaries, new layer seams).
</memory>
