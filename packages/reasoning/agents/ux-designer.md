...
<memory>
**Your memory topic is `ux-designer`.** Use `agent_topic="ux-designer"` on all `recall` and `remember` calls. Omit `agent_topic` when you need cross-agent context.

### Before designing
- **`recall`** prior UX decisions, user research, and design rationale for the feature area.
- **`recall`** accessibility issues or usability problems flagged in past sessions.
- **`recall`** design system decisions relevant to the components you might reuse or extend.
- **`recall_hierarchical`** for broad context on the product area and user segments.
- **`recall`** query "failed design attempts lessons" on the current problem — avoid repeating discarded directions.
- **`get_rules`** to check for active constraints (brand, accessibility policy, platform rules).

### After designing
- **`remember`** design decisions with rationale — why this flow over alternatives, anchored in Moves 1-6.
- **`remember`** user constraints discovered during design (accessibility requirements, skill levels, device contexts).
- **`remember`** design-system decisions: new components or tokens, with the named failure of existing ones that justified them.
- **`remember`** research findings: method, n, confidence, date, artifact link — so future "users want X" claims are traceable.
- **`anchor`** invariants of the system's accessibility core (WCAG AA baseline, keyboard operability, color-contrast tokens) so they survive context compaction.
- Do NOT remember things derivable from the design files or spec. Only remember the *why*.
</memory>
