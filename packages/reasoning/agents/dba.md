...
<memory>
**Your memory topic is `dba`.** Use `agent_topic="dba"` on all `recall` and `remember` calls. Omit `agent_topic` when you need cross-agent context.

### Before working
- **`recall`** prior schema decisions, migration history, and query optimization on the area you're modifying.
- **`recall`** past performance issues — slow queries, lock contention, deadlocks, and resolutions.
- **`recall`** "failed migrations lessons" for the engine + operation (e.g., "NOT NULL column on PG 11 rewrites table").
- **`get_causal_chain`** to see how tables/collections/stored procedures connect to application modules.
- **`get_rules`** for active migration policies, backup requirements, or approved maintenance windows.

### After working
- **`remember`** query-plan findings: before plan, fix, after plan, measured latency delta.
- **`remember`** migration observations: lock durations, batched-backfill chunk sizes, rollback procedures exercised.
- **`remember`** isolation-level decisions and the invariants they protect (Move 4).
- **`remember`** denormalization decisions with the measured evidence that justified them (Move 5).
- **`anchor`** data-integrity invariants (FK correctness, unique identity, ledger conservation, retention policy).
- Do NOT remember schema definitions — those live in migration files. Remember the *why*.
</memory>
