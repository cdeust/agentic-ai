 ...
 *
 * Liskov audit 2026-05-04 — two FAIL findings corrected:
 *   FAIL-1: 5 missing response fields restored:
 *     - status      ("ok" | "error")
 *     - sources     (list of contributing signal names)
 *     - counts      ({ap: number, cortex: number})
 *     - k           (RRF k parameter used)
 *     - AP-search leg (RRF-fuse Cortex memory + AP code symbols)
 *   FAIL-2: AP-search leg absent. Restored by calling CodebasePort.searchCodebase
 *     when codebasePort is provided. Graceful degradation when absent.
 *
 * RRF formula: score(d) = Σ_r 1 / (k + rank_r(d))
 * source: Cormack, Clarke, Büttcher (2009) SIGIR — K=60 canonical value.
...
// source: cortex@ed33435 unified_search.py — ap_limit default 20
...
export const UnifiedSearchRequestSchema = z.object({
  ...
  // source: Cormack, Clarke, Büttcher (2009) SIGIR — K=60 canonical
...
export interface UnifiedSearchResponse {
  ...
  query: string;
  total: number;
  /** Ranking signal names contributed to RRF fusion. */
  sources: string[];
  /** Per-source result counts before fusion. */
...
export interface UnifiedSearchDeps {
  ...
  /**
   * Optional CodebasePort for AP symbol search.
   * When absent, AP leg skipped (graceful degradation).
...
/**
 * Score a memory record against a query (binary keyword overlap / doc length).
 *
 * precondition:  query non-empty; record has non-empty content.
 * postcondition: float in [0, 1]; 1 = maximum overlap.
 *
 * source: Cormack et al. (2009) SIGIR — RRF is robust to imperfect base rankers.
 */
 ...
 *   sources = list of ranking signal names contributed.
...
export async function unifiedSearchHandler(
  ...
  // AP code search leg — source: cortex@ed33435 unified_search.py AP leg guard
  ...
  if (codebasePort !== undefined && graph_path !== undefined && graph_path.length > 0) {
    try {
      ...
      // AP leg failure is non-fatal — graceful degradation.
      // source: cortex@ed33435 unified_search.py — ap_client call wrapped in try/except
    }
  }

  // RRF fusion — source: Cormack et al. (2009) SIGIR
