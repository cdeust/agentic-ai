...
type FailureCategory =
  | "missing_architecture"   // No ports/adapters, no clean arch
  | "missing_security"       // No auth, no validation, no encryption
  | "missing_data_model"     // Orphan DDL, missing FK references
  | "missing_test_evidence"  // Placeholder tests, no traceability
  | "structural_arithmetic"  // SP arithmetic, numbering gaps
  | "missing_resilience"     // No error handling, no rollback
  | "ungrounded_claim"       // Metrics without disclaimer, NFRs all PASS
  | "naming_convention"      // AnyCodable, Date() in domain, nested types
  | "missing_observability"  // No logging, no tracing, no alerting
  | "uncategorized";
