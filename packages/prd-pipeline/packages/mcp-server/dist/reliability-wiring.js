...
// ─── Lazy singleton ───────────────────────────────────────────────────────────
let _reliabilityRepo = undefined;
let _reliabilityProvider = undefined;
/**
 ...
 */
export function getReliabilityRepo() {
 ...
 */
export function getConsensusReliabilityProvider() {
 ...
 */
export function closeReliabilityRepo() {
 ...
 */
export function checkReliabilityHealth() {
    const repo = getReliabilityRepo();
    if (repo === null) {
        return {
            healthy: false,
            schemaVersion: null,
            recordCount: null,
            message: "reliability.db unavailable: better-sqlite3 not installed or DB failed to open. " +
    ...
    try {
        ...
        return {
            ...
            schemaVersion: null,
            recordCount: null,
            message: `reliability.db schema check failed: ${String(err)}. ` +
    ...
    if (schemaVersion !== RELIABILITY_SCHEMA_VERSION) {
        return {
            ...
            schemaVersion,
            recordCount: null,
            message: `reliability.db schema version mismatch: DB has version ${schemaVersion}, ` +
    ...
    if (recordCount === 0) {
        ...
        console.error("[prd-gen] reliability.db: no calibrated reliabilities yet — " +
...
//# sourceMappingURL=reliability-wiring.js.map