 ...
 *   2. Delegates tool registration to one file per topic in src/tools/.
 *   3. Connects the server to StdioServerTransport.
 *   4. Keeps this file under 50 lines of logic — all substance is in tools/.
 *
 * All 46 tools from MCP_TOOLS.md are registered by the register* functions.
 * The tool adapter stubs return stub JSON; a future Phase-6 wiring step will
 * inject MemoryStore, ProfilesStore, and WikiStore adapters at startup.
 *
...
registerNarrativeTools(server);    // 3 tools
